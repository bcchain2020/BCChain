...
#include <bcio/wallet_plugin/macos_user_auth.h>
...
#define STR(A) #A
   ...
   bool remove_key(string public_key) {
      ...
      macos_user_auth(auth_callback, &prom, CFSTR("remove a key from your BCIO wallet"));
      if(!fut.get())
...
static void check_signed() {
   ...
   if(is_valid != errSecSuccess) {
      ...
      BC_THROW(secure_enclave_exception, "");
   }
}
...
se_wallet::se_wallet() : my(new detail::se_wallet_impl()) {
   ...
   BC_THROW(secure_enclave_exception, "Secure Enclave not supported on this hardware");
}
...
void se_wallet::lock() {
   BC_ASSERT(!is_locked(), wallet_locked_exception, "You can not lock an already locked wallet");
   my->locked = true;
}
...
void se_wallet::unlock(string password) {
   promise<bool> prom;
   future<bool> fut = prom.get_future();
   macos_user_auth(detail::auth_callback, &prom, CFSTR("unlock your BCIO wallet"));
   if(!fut.get())
      FC_THROW_EXCEPTION(chain::wallet_invalid_password_exception, "Local user authentication failed");
   my->locked = false;
}
...
bool se_wallet::remove_key(string key) {
   BC_ASSERT(!is_locked(), wallet_locked_exception, "You can not remove a key from a locked wallet");
