...
#include <bcio/net_plugin/net_plugin.hpp>
#include <bcio/net_plugin/protocol.hpp>
...
using namespace bcio::chain::plugin_interface::compat;
...
namespace bcio {
   ...
   using bcio::chain::transaction_id_type;
...
#define peer_elog( PEER, FORMAT, ... ) \
  ...
  FC_MULTILINE_MACRO_END
   ...
   struct update_request_time {
      ...
      void operator () (struct bcio::peer_block_state &bs) {
         bs.requested_time = time_point::now();
      }
   } set_request_time;

   typedef multi_index_container<
      bcio::peer_block_state,
      indexed_by<
         ordered_unique< tag<by_id>, member<bcio::peer_block_state, block_id_type, &bcio::peer_block_state::id > >,
         ordered_unique< tag<by_block_num>, member<bcio::peer_block_state, uint32_t, &bcio::peer_block_state::block_num > >
         >
      > peer_block_state_index;


   struct update_known_by_peer {
      void operator() (bcio::peer_block_state& bs) {
         bs.is_known = true;
      }
      void operator() (transaction_state& ts) {
            ...
            [this,weak_conn]( boost::system::error_code ec, std::size_t bytes_transferred ) {
               ...
               try {
                  if( !ec ) {
                     ...
                     BC_ASSERT(bytes_transferred <= conn->pending_message_buffer.bytes_to_write(), plugin_exception, "");
   ...
   void net_plugin_impl::handle_message( connection_ptr c, const packed_transaction &msg) {
      ...
      controller& cc = my_impl->chain_plug->chain();
      if( cc.get_read_mode() == bcio::db_read_mode::READ_ONLY ) {
...
//      dispatcher->bcast_transaction(md->packed_trx);
   ...
   void net_plugin::set_program_options( options_description& /*cli*/, options_description& cfg )
   {
      cfg.add_options()
         ...
         ( "agent-name", bpo::value<string>()->default_value("\"BC Test Agent\""), "The name supplied to identify this node amongst the peers.")
   ...
   void net_plugin::plugin_initialize( const variables_map& options ) {
      ...
      try {
         ...
         if( my->allowed_connections & net_plugin_impl::Specified )
            BC_ASSERT( options.count( "peer-key" ),
