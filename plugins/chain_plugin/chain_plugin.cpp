...
#include <bcio/chain/fork_database.hpp>
#include <bcio/chain/block_log.hpp>
#include <bcio/chain/exceptions.hpp>
#include <bcio/chain/authorization_manager.hpp>
#include <bcio/chain/producer_object.hpp>
#include <bcio/chain/config.hpp>
#include <bcio/chain/wasm_interface.hpp>
#include <bcio/chain/resource_limits.hpp>
#include <bcio/chain/reversible_block_object.hpp>
#include <bcio/chain/controller.hpp>
#include <bcio/chain/generated_transaction_object.hpp>
#include <bcio/chain/snapshot.hpp>

#include <bcio/chain/bcio_contract.hpp>


#include <bcio/utilities/key_conversion.hpp>
#include <bcio/utilities/common.hpp>
...
namespace bcio {
	...
	namespace chain {
		...
		std::ostream& operator<<(std::ostream& osm, bcio::chain::db_read_mode m) {
			if (m == bcio::chain::db_read_mode::SPECULATIVE) {
				osm << "speculative";
			}
			else if (m == bcio::chain::db_read_mode::HEAD) {
				osm << "head";
			}
			else if (m == bcio::chain::db_read_mode::READ_ONLY) {
				osm << "read-only";
			}
			else if (m == bcio::chain::db_read_mode::IRREVERSIBLE) {
				osm << "irreversible";
			}
		...
		void validate(boost::any& v,
			...
			bcio::chain::db_read_mode* /* target_type */,
			int)
		{
			...
			if (s == "speculative") {
				v = boost::any(bcio::chain::db_read_mode::SPECULATIVE);
			}
			else if (s == "head") {
				v = boost::any(bcio::chain::db_read_mode::HEAD);
			}
			else if (s == "read-only") {
				v = boost::any(bcio::chain::db_read_mode::READ_ONLY);
			}
			else if (s == "irreversible") {
				v = boost::any(bcio::chain::db_read_mode::IRREVERSIBLE);
			}
			else {
		...
		std::ostream& operator<<(std::ostream& osm, bcio::chain::validation_mode m) {
			if (m == bcio::chain::validation_mode::FULL) {
				osm << "full";
			}
			else if (m == bcio::chain::validation_mode::LIGHT) {
				osm << "light";
			}
		...
		void validate(boost::any& v,
			const std::vector<std::string>& values,
			bcio::chain::validation_mode* /* target_type */,
			int)
		{
			using namespace boost::program_options;
			...
			if (s == "full") {
				v = boost::any(bcio::chain::validation_mode::FULL);
			}
			else if (s == "light") {
				v = boost::any(bcio::chain::validation_mode::LIGHT);
			}
			else {
				throw validation_error(validation_error::invalid_option_value);
			}
	...
	using namespace bcio::chain::config;
	using namespace bcio::chain::plugin_interface;
...
#define CATCH_AND_CALL(NEXT)\
	...
	void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
	{
		cfg.add_options()
			...
			("wasm-runtime", bpo::value<bcio::chain::wasm_interface::vm_type>()->value_name("wavm/wabt"), "Override default WASM runtime")
							...
							("contract-blacklist", boost::program_options::value<vector<string>>()->composing()->multitoken(),
								...
								("action-blacklist", boost::program_options::value<vector<string>>()->composing()->multitoken(),
									...
									("key-blacklist", boost::program_options::value<vector<string>>()->composing()->multitoken(),
										...
										("read-mode", boost::program_options::value<bcio::chain::db_read_mode>()->default_value(bcio::chain::db_read_mode::SPECULATIVE),
			...
			//"In \"irreversible\" mode database contains changes done up the current irreversible block.\n")
											("validation-mode", boost::program_options::value<bcio::chain::validation_mode>()->default_value(bcio::chain::validation_mode::FULL),
...
if( options.count(name) ) { \
   ...
   std::copy(ops.begin(), ops.end(), std::inserter(container, container.end())); \
	...
	void chain_plugin::plugin_initialize(const variables_map& options) {
		...
		try {
			try {
				genesis_state gs; // Check if BCIO_ROOT_KEY is bad
			}
			catch (const fc::exception&) {
				elog("BCIO_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
					("root_key", genesis_state::bcio_root_key));
				throw;
			}
			...
			if (options.count("action-blacklist")) {
				...
				for (const auto& a : acts) {
					auto pos = a.find("::");
					BC_ASSERT(pos != std::string::npos, plugin_config_exception, "Invalid entry in action-blacklist: '${a}'", ("a", a));
			...
			if (options.count("checkpoint")) {
				...
				for (const auto& cp : cps) {
					...
					if (itr != my->loaded_checkpoints.end()) {
						BC_ASSERT(itr->second == item.second,
			...
			if (options.count("extract-genesis-json") || options.at("print-genesis-json").as<bool>()) {
				...
				BC_THROW(extract_genesis_state_exception, "extracted genesis state from blocks.log");
			}
			...
			if (options.count("export-reversible-blocks")) {
				...
				BC_THROW(node_management_success, "exported reversible blocks");
			}
			...
			if (options.at("delete-all-blocks").as<bool>()) {
				...
				BC_THROW(fixed_reversible_db_exception, "fixed corrupted reversible blocks database");
			}
				...
				BC_THROW(node_management_success, "imported reversible blocks");
			}
			...
			if (options.count("snapshot")) {
				...
				BC_ASSERT(fc::exists(*my->snapshot_path), plugin_config_exception,
				...
				BC_ASSERT(options.count("genesis-json") == 0 && options.count("genesis-timestamp") == 0,
					plugin_config_exception,
				...
				BC_ASSERT(!fc::exists(shared_mem_path),
					plugin_config_exception,
				...
				if (fc::is_regular_file(my->blocks_dir / "blocks.log")) {
					...
					BC_ASSERT(log_genesis.compute_chain_id() == my->chain_config->genesis.compute_chain_id(),
						plugin_config_exception,
				...
				if (options.count("genesis-json")) {
					BC_ASSERT(!fc::exists(my->blocks_dir / "blocks.log"),
						plugin_config_exception,
					...
					BC_ASSERT(fc::is_regular_file(genesis_file),
						plugin_config_exception,
			...
			if (options.count("read-mode")) {
				...
				BC_ASSERT(my->chain_config->read_mode != db_read_mode::IRREVERSIBLE, plugin_config_exception, "irreversible mode not currently supported.");
			}
			...
			my->pre_accepted_block_connection = my->chain->pre_accepted_block.connect([this](const signed_block_ptr& blk) {
				...
				if (itr != my->loaded_checkpoints.end()) {
					auto id = blk->id();
					BC_ASSERT(itr->second == id, checkpoint_exception,
	...
	void chain_apis::read_write::validate() const {
		BC_ASSERT(db.get_read_mode() != chain::db_read_mode::READ_ONLY, missing_chain_api_plugin_exception, "Not allowed, node in read-only mode");
	}
	...
	bool chain_plugin::recover_reversible_blocks(const fc::path& db_dir, uint32_t cache_size,
		...
		if (new_db_dir) {
			...
			BC_ASSERT(reversible_dir_name != ".", invalid_reversible_blocks_dir, "Invalid path to reversible directory");
			backup_dir = reversible_dir.parent_path() / reversible_dir_name.append("-").append(now);

			BC_ASSERT(!fc::exists(backup_dir),
		...
		try {
			for (; itr != ubi.end(); ++itr) {
				BC_ASSERT(itr->blocknum == end + 1, gap_in_reversible_blocks_db,
		...
		try {
			while (reversible_blocks.tellg() < end_pos) {
				...
				if (start == 0) {
					...
					BC_ASSERT(num == end + 1, gap_in_reversible_blocks_db,
	...
	chain::chain_id_type chain_plugin::get_chain_id()const {
		BC_ASSERT(my->chain_id.valid(), chain_id_type_exception, "chain ID has not been initialized yet");
		return *my->chain_id;
	}
	...
	void chain_plugin::handle_db_exhaustion() {
		...
		//return 1 -- it's what programs/nodbc/main.cpp considers "BAD_ALLOC"
		std::_Exit(1);
	}
	...
	namespace chain_apis {
		...
		read_only::get_info_results read_only::get_info(const read_only::get_info_params&) const {
			...
			return {
			   bcio::utilities::common::itoh(static_cast<uint32_t>(app().version())),
			   db.get_chain_id(),
		...
		uint64_t read_only::get_table_index_name(const read_only::get_table_rows_params& p, bool& primary) {
			...
			BC_ASSERT(index == table, chain::contract_table_query_exception, "Unsupported table name: ${n}", ("n", p.table));
			...
			if (p.index_position.empty() || p.index_position == "first" || p.index_position == "primary" || p.index_position == "one") {
				...
				try {
					...
					BC_ASSERT(false, chain::contract_table_query_exception, "Invalid index_position: ${p}", ("p", p.index_position));
				}
				if (pos < 2) {
					primary = true;
					pos = 0;
		...
		uint64_t convert_to_type(const string& str, const string& desc) {
			...
			if (str.find(',') != string::npos) { // fix #6274 only match formats like 4,BC
				try {
					auto symb = bcio::chain::symbol::from_string(str);
					return symb.value();
				}
				catch (...) {}
			}
			...
			try {
				return (bcio::chain::string_to_symbol(0, str.c_str()) >> 8);
			}
			catch (...) {
				BC_ASSERT(false, chain_type_exception, "Could not convert ${desc} string '${str}' to any of the following: "
		...
		abi_def get_abi(const controller& db, const name& account) {
			...
			BC_ASSERT(code_accnt != nullptr, chain::account_query_exception, "Fail to retrieve account for ${account}", ("account", account));
			abi_def abi;
		...
		string get_table_type(const abi_def& abi, const name& table_name) {
			...
			BC_ASSERT(false, chain::contract_table_query_exception, "Table ${table} is not specified in the ABI", ("table", table_name));
		}

		read_only::get_table_rows_result read_only::get_table_rows(const read_only::get_table_rows_params& p)const {
			const abi_def abi = bcio::chain_apis::get_abi(db, p.code);

			bool primary = false;
			auto table_with_index = get_table_index_name(p, primary);
			if (primary) {
				BC_ASSERT(p.table == table_with_index, chain::contract_table_query_exception, "Invalid table name ${t}", ("t", p.table));
				auto table_type = get_table_type(abi, p.table);
				if (table_type == KEYi64 || p.key_type == "i64" || p.key_type == "name") {
					return get_table_rows_ex<key_value_index>(p, abi);
				}
				BC_ASSERT(false, chain::contract_table_query_exception, "Invalid table type ${type}", ("type", table_type)("abi", abi));
			}
			else {
				BC_ASSERT(!p.key_type.empty(), chain::contract_table_query_exception, "key type required for non-primary index");
				...
				BC_ASSERT(false, chain::contract_table_query_exception, "Unsupported secondary index type: ${t}", ("t", p.key_type));
			}
		}
		...
		vector<asset> read_only::get_currency_balance(const read_only::get_currency_balance_params& p)const {
			...
			walk_key_value_table(p.code, p.account, N(accounts), [&](const key_value_object& obj) {
				BC_ASSERT(obj.value.size() >= sizeof(asset), chain::asset_type_exception, "Invalid data on table");

				asset cursor;
				fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
				fc::raw::unpack(ds, cursor);

				BC_ASSERT(cursor.get_symbol().valid(), chain::asset_type_exception, "Invalid asset");
		...
		fc::variant read_only::get_currency_stats(const read_only::get_currency_stats_params& p)const {
			...
			uint64_t scope = (bcio::chain::string_to_symbol(0, boost::algorithm::to_upper_copy(p.symbol).c_str()) >> 8);

			walk_key_value_table(p.code, scope, N(stat), [&](const key_value_object& obj) {
				BC_ASSERT(obj.value.size() >= sizeof(read_only::get_currency_stats_result), chain::asset_type_exception, "Invalid data on table");
		...
		fc::variant get_global_row(const database& db, const abi_def& abi, const abi_serializer& abis, const fc::microseconds& abi_serializer_max_time_ms, bool shorten_abi_errors) {
			...
			BC_ASSERT(table_type == read_only::KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table global", ("type", table_type));
			...
			BC_ASSERT(table_id, chain::contract_table_query_exception, "Missing table global");
			...
			BC_ASSERT(it != kv_index.end(), chain::contract_table_query_exception, "Missing row in table global");
		...
		read_only::get_producers_result read_only::get_producers(const read_only::get_producers_params& p) const {
			const abi_def abi = bcio::chain_apis::get_abi(db, config::system_account_name);
			const auto table_type = get_table_type(abi, N(producers));
			const abi_serializer abis{ abi, abi_serializer_max_time };
			BC_ASSERT(table_type == KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table producers", ("type", table_type));
			...
			BC_ASSERT(table_id && secondary_table_id, chain::contract_table_query_exception, "Missing producers table");
				...
				if (!p.lower_bound.empty()) {
					try {
						...
						try {
							...
							if (itr == by_txid.end()) {
								BC_THROW(transaction_exception, "Unknown Transaction ID: ${txid}", ("txid", txid));
							}
		...
		fc::variant read_only::get_block(const read_only::get_block_params& params) const {
		    ...
		    BC_ASSERT(!params.block_num_or_id.empty() && params.block_num_or_id.size() <= 64, chain::block_id_type_exception, "Invalid Block number or ID, must be greater than 0 and less than 64 characters");
			try {
			...
			} BC_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))

				BC_ASSERT(block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));
		...
		fc::variant read_only::get_block_header_state(const get_block_header_state_params& params) const {
			...
			BC_ASSERT(b, unknown_block_exception, "Could not find reversible block: ${block}", ("block", params.block_num_or_id));
		...
		void read_write::push_transaction(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
			...
			try {
				...
				} BC_RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid packed transaction")
		...
		void read_write::push_transactions(const read_write::push_transactions_params& params, next_function<read_write::push_transactions_results> next) {
			try {
				BC_ASSERT(params.size() <= 1000, too_many_tx_at_once, "Attempt to push too many transactions at once");
		...
		read_only::get_code_results read_only::get_code(const get_code_params& params)const {
			...
			BC_ASSERT(params.code_as_wasm, unsupported_feature, "Returning WAST from get_code is no longer supported");
		...
		read_only::get_account_results read_only::get_account(const get_account_params& params)const {
			...
			while (perm != permissions.end() && perm->owner == params.account_name) {
				...
				if (perm->parent._id) {
					...
					if (p) {
						BC_ASSERT(perm->owner == p->owner, invalid_parent_permission, "Invalid parent permission");
						parent = p->name;
					}
				}
			...
			if (abi_serializer::to_abi(code_account.abi, abi)) {
				...
				const auto token_code = N(bcio.token);
		...
		read_only::abi_json_to_bin_result read_only::abi_json_to_bin(const read_only::abi_json_to_bin_params& params)const try {
			...
			BC_ASSERT(code_account != nullptr, contract_query_exception, "Contract can't be found ${contract}", ("contract", params.code));

			abi_def abi;
			if (abi_serializer::to_abi(code_account->abi, abi)) {
				abi_serializer abis(abi, abi_serializer_max_time);
				auto action_type = abis.get_action_type(params.action);
				BC_ASSERT(!action_type.empty(), action_validate_exception, "Unknown action ${action} in contract ${contract}", ("action", params.action)("contract", params.code));
				try {
					result.binargs = abis.variant_to_binary(action_type, params.args, abi_serializer_max_time, shorten_abi_errors);
				} BC_RETHROW_EXCEPTIONS(chain::invalid_action_args_exception,
					"'${args}' is invalid args for action '${action}' code '${code}'. expected '${proto}'",
					("args", params.args)("action", params.action)("code", params.code)("proto", action_abi_to_variant(abi, action_type)))
			}
			else {
				BC_ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", params.code));
			}
			return result;
		...
		read_only::get_required_keys_result read_only::get_required_keys(const get_required_keys_params& params)const {
			...
			} BC_RETHROW_EXCEPTIONS(chain::transaction_type_exception, "Invalid transaction")
		...
		chain::symbol read_only::extract_core_symbol()const {
			...
			// The following code makes assumptions about the contract deployed on bcio account (i.e. the system contract) and how it stores its data.
			const auto& d = db.db();
			const auto* t_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple(N(bcio), N(bcio), N(rammarket)));
			if (t_id != nullptr) {
				const auto &idx = d.get_index<key_value_index, by_scope_primary>();
				auto it = idx.find(boost::make_tuple(t_id->id, bcio::chain::string_to_symbol_c(4, "RAMCORE")));
				if (it != idx.end()) {
...
FC_REFLECT(bcio::chain_apis::detail::ram_market_exchange_state_t, (ignore1)(ignore2)(ignore3)(core_symbol)(ignore4))
