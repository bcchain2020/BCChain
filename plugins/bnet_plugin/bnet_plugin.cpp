...
#include <bcio/bnet_plugin/bnet_plugin.hpp>
...
using bcio::public_key_type;
using bcio::chain_id_type;
using bcio::block_id_type;
using bcio::block_timestamp_type;
using std::string;
using bcio::sha256;
using bcio::signed_block_ptr;
using bcio::packed_transaction_ptr;
using std::vector;
...
namespace bcio {
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        void set_socket_options() {
           try {
             ...
             * wait for an "ack", making this larger could result in hibcr latency for smaller urgent
             * messages.
             */
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              BC_THROW(block_validate_exception, "bad block" );
           }
        ...
        void on( const packed_transaction_ptr& p ) {
           ...
           if (!p) {
              ...
              BC_THROW(transaction_exception, "bad transaction");
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           BC_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         BC_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
