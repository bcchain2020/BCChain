...
// given a baseline order, adjust it based on an important order of priorities (hibcr values
// are hibcr priority). The priorities take precedence, unless they are equal and then
...
void CoalesceLocalsWithLearning::pickIndices(std::vector<Index>& indices) {
  ...
  struct Generator {
    ...
    void calculateFitness(Order* order) {
      ...
      double fitness = parent->numLocals - maxIndex; // hibcr fitness is better
