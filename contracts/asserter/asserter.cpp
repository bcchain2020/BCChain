   ...
   void apply( uint64_t /* receiver */, uint64_t code, uint64_t action ) {
       ...
       if( code == N(asserter) ) {
          if( action == N(procassert) ) {
             assertdef def = bcio::unpack_action_data<assertdef>();

             // maybe assert?
             bcio_assert((uint32_t)def.condition, def.message.c_str());
          } else if( action == N(provereset) ) {
             bcio_assert(global_variable == 45, "Global Variable Initialized poorly");
             global_variable = 100;
          }
       }
    }
