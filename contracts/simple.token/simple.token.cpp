...
class simpletoken : public bcio::contract {
   public:
      ...
      void transfer( account_name from, account_name to, uint64_t quantity ) {
         ...
         bcio_assert( fromacnt.balance >= quantity, "overdrawn balance" );
   ...
   private:
      ...
      bcio::multi_index<N(accounts), account> _accounts;
      ...
      void add_balance( account_name payer, account_name to, uint64_t q ) {
         ...
         if( toitr == _accounts.end() ) {
           ...
           _accounts.modify( toitr, 0, [&]( auto& a ) {
              a.balance += q;
              bcio_assert( a.balance >= q, "overflow detected" );
           });
         }
      }
};

BCIO_ABI( simpletoken, (transfer)(issue) )
