...
namespace bcio {
    ...
    *    int  c; //4
    *
    *    BCLIB_SERIALIZE( dummy_action, (a)(b)(c) )
    *  };
   ...
   struct permission_level {
      ...
      BCLIB_SERIALIZE( permission_level, (actor)(permission) )
   };
   ...
   struct action {
      ...
      BCLIB_SERIALIZE( action, (account)(name)(authorization)(data) )
      ...
      void send_context_free() const {
         bcio_assert( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
      ...
      template<typename T>
      T data_as() {
         bcio_assert( name == T::get_name(), "Invalid name" );
         bcio_assert( account == T::get_account(), "Invalid account" );
...
::bcio::inline_dispatcher<decltype(&CONTRACT_CLASS::FUNCTION_NAME), ACTION_NAME>::call

#define INLINE_ACTION_SENDER2( CONTRACT_CLASS, NAME )\
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::bcio::string_to_name(#NAME) )
 ...
 * @param NAME - The name of the action
 */
#define ACTION( CODE, NAME ) struct NAME : ::bcio::action_meta<CODE, ::bcio::string_to_name(#NAME) >
