...
namespace bcio {
    ...
    * ```
    *
    * For this to work the Actions must be derived from bcio::contract
    *
    */
   ...
   bool dispatch( uint64_t code, uint64_t act ) {
      ...
      return bcio::dispatch<Contract,SecondAction,Actions...>( code, act );
   }
    ...
    * @tparam T - The contract class that has the correponding action handler, this contract should be derived from bcio::contract
...
// Helper macro for BCIO_API
#define BCIO_API_CALL( r, OP, elem ) \
   case ::bcio::string_to_name( BOOST_PP_STRINGIZE(elem) ): \
      bcio::execute_action( &thiscontract, &OP::elem ); \
      break;

// Helper macro for BCIO_ABI
#define BCIO_API( TYPE,  MEMBERS ) \
   BOOST_PP_SEQ_FOR_EACH( BCIO_API_CALL, TYPE, MEMBERS )
 ...
 * To be able to use this macro, the contract needs to be derived from bcio::contract
 * 
 ...
 * Example:
 * @code
 * BCIO_ABI( bcio::bios, (setpriv)(setalimits)(setglimits)(setprods)(reqauth) )
 * @endcode
 */
#define BCIO_ABI( TYPE, MEMBERS ) \
extern "C" { \
   void apply( uint64_t receiver, uint64_t code, uint64_t action ) { \
      auto self = receiver; \
      if( action == N(onerror)) { \
         /* onerror is only valid if it is for the "bcio" code account and authorized by "bcio"'s "active permission */ \
         bcio_assert(code == N(bcio), "onerror action's are only valid from the \"bcio\" system account"); \
      } \
      if( code == self || action == N(onerror) ) { \
         TYPE thiscontract( self ); \
         switch( action ) { \
            BCIO_API( TYPE, MEMBERS ) \
         } \
         /* does not allow destructor of thiscontract to run: bcio_exit(0); */ \
      } \
   } \
} \
 /// @}  dispatcher
