...
namespace bcio {
   ...
   void set_blockchain_parameters(const bcio::blockchain_parameters& params) {
      char buf[sizeof(bcio::blockchain_parameters)];
      bcio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(bcio::blockchain_parameters& params) {
      char buf[sizeof(bcio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      bcio_assert( size <= sizeof(buf), "buffer is too small" );
      bcio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            bcio_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return bcio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = bcio::memory_heap.malloc(count*size);
...
{
   return bcio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return bcio::memory_heap.free(ptr);
}
