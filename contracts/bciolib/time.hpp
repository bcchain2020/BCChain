...
namespace bcio {
  class microseconds {
    public:
        ...
        int64_t _count;
        BCLIB_SERIALIZE( microseconds, (_count) )
    private:
  ...
  class time_point {
    public:
        ...
        BCLIB_SERIALIZE( time_point, (elapsed) )
  };
  ...
  class time_point_sec
  {
    public:
        ...
        operator time_point()const { return time_point( bcio::seconds( utc_seconds) ); }
        uint32_t sec_since_epoch()const { return utc_seconds; }

        time_point_sec operator = ( const bcio::time_point& t )
        {
          utc_seconds = uint32_t(t.time_since_epoch().count() / 1000000ll);
          return *this;
        }
        ...
        BCLIB_SERIALIZE( time_point_sec, (utc_seconds) )
  };
   ...
   class block_timestamp {
      public:
         ...
         block_timestamp next() const {
            bcio_assert( std::numeric_limits<uint32_t>::max() - slot >= 1, "block timestamp overflow" );
         ...
         BCLIB_SERIALIZE( block_timestamp, (slot) )
      private:
