...
namespace bcio {
    ...
    *  @deprecated This class is deprecated in favor of bcio.token in Dawn 3.0
    */
   class currency {
      public:
         ...
         struct create {
           ...
           // array<char,32>         issuer_agreement_hash;
            ...
            BCLIB_SERIALIZE( create, (issuer)(maximum_supply)(issuer_can_freeze)(issuer_can_recall)(issuer_can_whitelist) )
         };
         ...
         struct transfer
         {
            ...
            BCLIB_SERIALIZE( transfer, (from)(to)(quantity)(memo) )
         };
         ...
         struct issue {
            ...
            BCLIB_SERIALIZE( issue, (to)(quantity)(memo) )
         };
         ...
         struct fee_schedule {
            ...
            BCLIB_SERIALIZE( fee_schedule, (fee_per_length) )
         };
         ...
         struct account {
            ...
            BCLIB_SERIALIZE( account, (balance)(frozen)(whitelist) )
         };
         ...
         struct currency_stats {
            ...
            BCLIB_SERIALIZE( currency_stats, (supply)(max_supply)(issuer)(can_freeze)(can_recall)(can_whitelist)(is_frozen)(enforce_whitelist) )
         };

         typedef bcio::multi_index<N(accounts), account> accounts;
         typedef bcio::multi_index<N(stat), currency_stats> stats;
         ...
         bool apply( account_name contract, action_name act ) {
          ...
          void create_currency( const create& c ) {
            ...
            bcio_assert( sym.is_valid(), "invalid symbol name" );

             stats statstable( _contract, sym.name() );
             auto existing = statstable.find( sym.name() );
             bcio_assert( existing == statstable.end(), "token with symbol already exists" );
          ...
          void issue_currency( const issue& i ) {
             ...
             statstable.modify( st, 0, [&]( auto& s ) {
                ...
                bcio_assert( s.supply.amount >= 0, "underflow" );
             });
          ...
          void on( const issue& i ) {
             ...
             bcio_assert( i.quantity.is_valid(), "invalid quantity" );
             bcio_assert( i.quantity.amount > 0, "must issue positive quantity" );
          ...
          void on( const transfer& t ) {
             ...
             bcio_assert( t.quantity.is_valid(), "invalid quantity" );
             bcio_assert( t.quantity.amount > 0, "must transfer positive quantity" );
      ...
      private:
          void sub_balance( account_name owner, asset value, const currency_stats& st ) {
             ...
             bcio_assert( from.balance.amount >= value.amount, "overdrawn balance" );

             if( has_auth( owner ) ) {
                bcio_assert( !st.can_freeze || !from.frozen, "account is frozen by issuer" );
                bcio_assert( !st.can_freeze || !st.is_frozen, "all transfers are frozen by issuer" );
                bcio_assert( !st.enforce_whitelist || from.whitelist, "account is not white listed" );
             } else if( has_auth( st.issuer ) ) {
                bcio_assert( st.can_recall, "issuer may not recall token" );
             } else {
                bcio_assert( false, "insufficient authority" );
             }
          ...
          void add_balance( account_name owner, asset value, const currency_stats& st, account_name ram_payer )
          {
             ...
             if( to == to_acnts.end() ) {
                bcio_assert( !st.enforce_whitelist, "can only transfer to white listed accounts" );
                to_acnts.emplace( ram_payer, [&]( auto& a ){
                  a.balance = value;
                });
             } else {
                bcio_assert( !st.enforce_whitelist || to->whitelist, "receiver requires whitelist by issuer" );
