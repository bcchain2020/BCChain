...
#include <bciolib/compiler_builtins.h>
...
void test_action::read_action_normal() {
   ...
   bcio_assert(action_data_size() == sizeof(dummy_action), "action_size() == sizeof(dummy_action)");

   total = read_action_data(buffer, 30);
   bcio_assert(total == sizeof(dummy_action) , "read_action(30)" );

   total = read_action_data(buffer, 100);
   bcio_assert(total == sizeof(dummy_action) , "read_action(100)" );

   total = read_action_data(buffer, 5);
   bcio_assert(total == 5 , "read_action(5)" );

   total = read_action_data(buffer, sizeof(dummy_action) );
   bcio_assert(total == sizeof(dummy_action), "read_action(sizeof(dummy_action))" );

   dummy_action *dummy13 = reinterpret_cast<dummy_action *>(buffer);

   bcio_assert(dummy13->a == DUMMY_ACTION_DEFAULT_A, "dummy13->a == DUMMY_ACTION_DEFAULT_A");
   bcio_assert(dummy13->b == DUMMY_ACTION_DEFAULT_B, "dummy13->b == DUMMY_ACTION_DEFAULT_B");
   bcio_assert(dummy13->c == DUMMY_ACTION_DEFAULT_C, "dummy13->c == DUMMY_ACTION_DEFAULT_C");
}
...
void test_action::test_dummy_action() {
   ...
   bcio_assert( total > 0, "get_action failed" );
   bcio::action act = bcio::get_action( 1, 0 );
   bcio_assert( act.authorization.back().actor == N(testapi), "incorrect permission actor" );
   bcio_assert( act.authorization.back().permission == N(active), "incorrect permission name" );
   bcio_assert( bcio::pack_size(act) == static_cast<size_t>(total), "pack_size does not match get_action size" );
   bcio_assert( act.account == N(testapi), "expected testapi account" );
   ...
   if ( dum13.b == 200 ) {
      ...
      bcio_assert(false, "get_context_free_data() not allowed in non-context free action");
   } else {
      bcio_assert(dum13.a == DUMMY_ACTION_DEFAULT_A, "dum13.a == DUMMY_ACTION_DEFAULT_A");
      bcio_assert(dum13.b == DUMMY_ACTION_DEFAULT_B, "dum13.b == DUMMY_ACTION_DEFAULT_B");
      bcio_assert(dum13.c == DUMMY_ACTION_DEFAULT_C, "dum13.c == DUMMY_ACTION_DEFAULT_C");
   }
}
...
void test_action::test_cf_action() {
   ...
   bcio::action act = bcio::get_action( 0, 0 );
   ...
   if ( cfa.payload == 100 ) {
      ...
      bcio_assert( size > 0, "size determination failed" );
      bcio::bytes cfd( static_cast<size_t>(size) );
      size = get_context_free_data( cfa.cfd_idx, &cfd[0], static_cast<size_t>(size) );
      bcio_assert(static_cast<size_t>(size) == cfd.size(), "get_context_free_data failed" );
      uint32_t v = bcio::unpack<uint32_t>( &cfd[0], cfd.size() );
      bcio_assert( v == cfa.payload, "invalid value" );
      ...
      bcio::action dum_act;
      dum_act.send();
      bcio_assert( false, "action send should not be allowed" );
   } else if ( cfa.payload == 206 ) {
      bcio::require_auth(N(test));
      bcio_assert( false, "authorization_api should not be allowed" );
   } else if ( cfa.payload == 207 ) {
      now();
      bcio_assert( false, "system_api should not be allowed" );
      ...
      bcio_assert( false, "transaction_api should not be allowed" );
...
void test_action::require_notice(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if( receiver == N(testapi) ) {
      bcio::require_recipient( N(acc1) );
      bcio::require_recipient( N(acc2) );
      bcio::require_recipient( N(acc1), N(acc2) );
      bcio_assert(false, "Should've failed");
...
void test_action::require_notice_tests(uint64_t receiver, uint64_t code, uint64_t action) {
   bcio::print( "require_notice_tests" );
   if( receiver == N( testapi ) ) {
      bcio::print( "require_recipient( N(acc5) )" );
      bcio::require_recipient( N( acc5 ) );
   } else if( receiver == N( acc5 ) ) {
      bcio::print( "require_recipient( N(testapi) )" );
      bcio::require_recipient( N( testapi ) );
   }
}

void test_action::require_auth() {
   prints("require_auth");
   bcio::require_auth( N(acc3) );
   bcio::require_auth( N(acc4) );
}

void test_action::assert_false() {
   bcio_assert(false, "test_action::assert_false");
}

void test_action::assert_true() {
   bcio_assert(true, "test_action::assert_true");
}
...
void test_action::test_abort() {
   abort();
   bcio_assert( false, "should've aborted" );
}

void test_action::test_publication_time() {
   uint64_t pub_time = 0;
   uint32_t total = read_action_data(&pub_time, sizeof(uint64_t));
   bcio_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)");
   bcio_assert( pub_time == publication_time(), "pub_time == publication_time()" );
}
...
void test_action::test_current_receiver(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   bcio_assert( receiver == cur_rec, "the current receiver does not match" );
}
...
void test_action::test_current_time() {
   ...
   bcio_assert( total == sizeof(uint64_t), "total == sizeof(uint64_t)");
   bcio_assert( tmp == current_time(), "tmp == current_time()" );
}
...
void test_action::test_ram_billing_in_notify(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   bcio_assert( total == sizeof(uint128_t), "total == sizeof(uint128_t)");

   uint64_t to_notify = tmp >> 64;
   uint64_t payer = tmp & 0xFFFFFFFFFFFFFFFFULL;

   if( code == receiver ) {
      bcio::require_recipient( to_notify );
   } else {
      bcio_assert( to_notify == receiver, "notified recipient other than the one specified in to_notify" );
