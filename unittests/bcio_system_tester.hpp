...
namespace bcio_system {

class bcio_system_tester : public TESTER {
public:

   bcio_system_tester()
   : bcio_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   bcio_system_tester(Lambda setup) {
      setup(*this);
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(bcio.token) );
         abi_def abi;
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance("bcio")  + get_balance("bcio.ramfee") + get_balance("bcio.stake") + get_balance("bcio.ram") );
   }
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& bcin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",bcin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(bcio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = bcio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(bcio.token), symbol_code, N(stat), symbol_code );
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "bcio_global_state", data, abi_serializer_max_time );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(bcio.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "bcio", "bcio.msig", core_from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "bcio", "alice1111111", core_from_string("650000000.0000"), "bcio" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::bcio_code_name}, 1},
...
inline uint64_t M( const string& bc_str ) {
   return core_from_string( bc_str ).get_amount();
}
