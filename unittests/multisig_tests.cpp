...
#include <bcio.msig/bcio.msig.wast.hpp>
#include <bcio.msig/bcio.msig.abi.hpp>
...
class bcio_msig_tester : public tester {
public:

   bcio_msig_tester() {
      create_accounts( { N(bcio.msig), N(bcio.stake), N(bcio.ram), N(bcio.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      set_code( N(bcio.msig), bcio_msig_wast );
      set_abi( N(bcio.msig), bcio_msig_abi );

      produce_blocks();
      const auto& accnt = control->db().get<account_object,by_name>( N(bcio.msig) );
      abi_def abi;
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(bcio.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const string& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(bcio.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(bcio.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(bcio.msig), name, accounts, data );
      produce_block();
...
transaction bcio_msig_tester::reqauth( account_name from, const vector<permission_level>& auths, const fc::microseconds& max_serialization_time ) {
   fc::variants v;
...
BOOST_AUTO_TEST_SUITE(bcio_msig_tests)

BOOST_FIXTURE_TEST_CASE( propose_approve_execute, bcio_msig_tester ) try {
   ...
   BOOST_REQUIRE_EXCEPTION( push_action( N(alice), N(exec), mvo()
                            ...
                            bcio_assert_message_is("transaction authorization failed")
   );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_unapprove, bcio_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_by_two, bcio_msig_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( propose_with_wrong_requested_auth, bcio_msig_tester ) try {
   auto trx = reqauth("alice", vector<permission_level>{ { N(alice), config::active_name },  { N(bob), config::active_name } }, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( big_transaction, bcio_msig_tester ) try {
   vector<permission_level> perm = { { N(alice), config::active_name }, { N(bob), config::active_name } };
   auto wasm = wast_to_wasm( bcio_token_wast );
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, bcio_msig_tester ) try {

   // required to set up the link between (bcio active) and (bcio.prods active)
   //
   //                  bcio active
   //                       |
   //             bcio.prods active (2/3 threshold)
   ...
   set_authority(config::system_account_name, "active", authority(1,
      vector<key_weight>{{get_private_key("bcio", "active").get_public_key(), 1}},
   ...
   create_accounts( { N(bcio.token) } );
   set_code( N(bcio.token), bcio_token_wast );
   set_abi( N(bcio.token), bcio_token_abi );

   create_currency( N(bcio.token), config::system_account_name, core_from_string("10000000000.0000") );
   issue(config::system_account_name, core_from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
                        get_balance("bcio") + get_balance("bcio.ramfee") + get_balance("bcio.stake") + get_balance("bcio.ram") );

   set_code( config::system_account_name, bcio_system_wast );
   set_abi( config::system_account_name, bcio_system_abi );
   ...
   // execute by alice to replace the bcio system contract
   transaction_trace_ptr trace;
   control->applied_transaction.connect([&]( const transaction_trace_ptr& t) { if (t->scheduled) { trace = t; } } );
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), config::system_account_name, core_from_string("1.0000"), false ),
                            bcio_assert_message_exception, bcio_assert_message_is("Unknown Test")

   );
} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, bcio_msig_tester ) try {

   // set up the link between (bcio active) and (bcio.prods active)
   set_authority(config::system_account_name, "active", authority(1,
      vector<key_weight>{{get_private_key("bcio", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{config::producers_account_name, config::active_name}, 1}}), "owner",
      { { config::system_account_name, "active" } }, { get_private_key( config::system_account_name, "active" ) });
   ...
   create_currency( N(bcio.token), config::system_account_name, core_from_string("10000000000.0000") );
   issue(config::system_account_name, core_from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( "bcio" ) );
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      bcio_assert_message_exception, bcio_assert_message_is("transaction authorization failed")
   );
