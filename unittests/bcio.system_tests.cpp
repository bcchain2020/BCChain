...
#include <bcio/chain/contract_table_objects.hpp>
...
#include "bcio_system_tester.hpp"

using namespace bcio_system;

BOOST_AUTO_TEST_SUITE(bcio_system_tests)

BOOST_FIXTURE_TEST_CASE( buysell, bcio_system_tester ) try {

   BOOST_REQUIRE_EQUAL( core_from_string("0.0000"), get_balance( "alice1111111" ) );

   transfer( "bcio", "alice1111111", core_from_string("1000.0000"), "bcio" );
   BOOST_REQUIRE_EQUAL( success(), stake( "bcio", "alice1111111", core_from_string("200.0000"), core_from_string("100.0000") ) );

   auto total = get_total_stake( "alice1111111" );
   auto init_bytes =  total["ram_bytes"].as_uint64();

   const asset initial_ram_balance = get_balance(N(bcio.ram));
   const asset initial_ramfee_balance = get_balance(N(bcio.ramfee));
   ...
   BOOST_REQUIRE_EQUAL( initial_ram_balance + core_from_string("199.0000"), get_balance(N(bcio.ram)) );
   BOOST_REQUIRE_EQUAL( initial_ramfee_balance + core_from_string("1.0000"), get_balance(N(bcio.ramfee)) );
   ...
   transfer( "bcio", "alice1111111", core_from_string("100000000.0000"), "bcio" );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake, bcio_system_tester ) try {
   ...
   const auto init_bcio_stake_balance = get_balance( N(bcio.stake) );
   ...
   BOOST_REQUIRE_EQUAL( init_bcio_stake_balance + core_from_string("300.0000"), get_balance( N(bcio.stake) ) );
   ...
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL( core_from_string("1000.0000"), get_balance( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( init_bcio_stake_balance, get_balance( N(bcio.stake) ) );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_with_transfer, bcio_system_tester ) try {
   cross_15_percent_threshold();

   issue( "bcio", core_from_string("1000.0000"), config::system_account_name );
   issue( "bcio.stake", core_from_string("1000.0000"), config::system_account_name );
   BOOST_REQUIRE_EQUAL( core_from_string("0.0000"), get_balance( "alice1111111" ) );

   //bcio stakes for alice with transfer flag

   transfer( "bcio", "bob111111111", core_from_string("1000.0000"), "bcio" );
   ...
   transfer( "bcio", "alice1111111", core_from_string("1000.0000"), "bcio" );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "alice1111111", core_from_string("200.0000"), core_from_string("100.0000") ) );
   //now alice's stake should be equal to transfered from bcio + own stake
   total = get_total_stake("alice1111111");
   BOOST_REQUIRE_EQUAL( core_from_string("700.0000"), get_balance( "alice1111111" ) );
...
BOOST_FIXTURE_TEST_CASE( stake_to_self_with_transfer, bcio_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( stake_while_pending_refund, bcio_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( fail_without_auth, bcio_system_tester ) try {
   cross_15_percent_threshold();

   issue( "alice1111111", core_from_string("1000.0000"),  config::system_account_name );

   BOOST_REQUIRE_EQUAL( success(), stake( "bcio", "alice1111111", core_from_string("2000.0000"), core_from_string("1000.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( stake_negative, bcio_system_tester ) try {
   issue( "alice1111111", core_from_string("1000.0000"),  config::system_account_name );
...
BOOST_FIXTURE_TEST_CASE( unstake_negative, bcio_system_tester ) try {
   issue( "alice1111111", core_from_string("1000.0000"),  config::system_account_name );
...
BOOST_FIXTURE_TEST_CASE( unstake_more_than_at_stake, bcio_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( delegate_to_another_user, bcio_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_separate, bcio_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( adding_stake_partial_unstake, bcio_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( stake_from_refund, bcio_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( stake_to_another_user_not_from_refund, bcio_system_tester ) try {
   cross_15_percent_threshold();
...
// Tests for voting
BOOST_FIXTURE_TEST_CASE( producer_register_unregister, bcio_system_tester ) try {
   issue( "alice1111111", core_from_string("1000.0000"),  config::system_account_name );

   //fc::variant params = producer_parameters_example(1);
   auto key =  fc::crypto::public_key( std::string("BC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV") );
   ...
   auto key2 =  fc::crypto::public_key( std::string("BC5jnmSKrzdBHE9n8hw58y7yxFWBC8SNiG7m8S1crJH3KvAnf9o6") );
   BOOST_REQUIRE_EQUAL( success(), push_action(N(alice1111111), N(regproducer), mvo()
                                               ("producer",  "alice1111111")
                                               ("producer_key", key2 )
                                               ("url", "http://block.two")
...
BOOST_FIXTURE_TEST_CASE( vote_for_producer, bcio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   cross_15_percent_threshold();
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "carol1111111", core_from_string("2.0000"), core_from_string("0.0002")/*"2.0000 BC", "0.0002 BC"*/ ) );
...
BOOST_FIXTURE_TEST_CASE( unregistered_producer_voting, bcio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   issue( "bob111111111", core_from_string("2000.0000"),  config::system_account_name );
...
BOOST_FIXTURE_TEST_CASE( more_than_30_producer_voting, bcio_system_tester ) try {
   issue( "bob111111111", core_from_string("2000.0000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "bob111111111", core_from_string("13.0000"), core_from_string("0.5791") ) );
   REQUIRE_MATCHING_OBJECT( voter( "bob111111111", core_from_string("13.5791") ), get_voter_info( "bob111111111" ) );
...
BOOST_FIXTURE_TEST_CASE( vote_same_producer_30_times, bcio_system_tester ) try {
   issue( "bob111111111", core_from_string("2000.0000"),  config::system_account_name );
...
BOOST_FIXTURE_TEST_CASE( producer_keep_votes, bcio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   issue( "alice1111111", core_from_string("1000.0000"),  config::system_account_name );
   fc::variant params = producer_parameters_example(1);
...
BOOST_FIXTURE_TEST_CASE( vote_for_two_producers, bcio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   //alice1111111 becomes a producer
   fc::variant params = producer_parameters_example(1);
...
BOOST_FIXTURE_TEST_CASE( proxy_register_unregister_keeps_stake, bcio_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( proxy_stake_unstake_keeps_proxy_flag, bcio_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( proxy_actions_affect_producers, bcio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE(producer_pay, bcio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // produce enough blocks so new schedule kicks in and defproducera produces some blocks
   {
      ...
      const int64_t  initial_savings           = get_balance(N(bcio.saving)).get_amount();
      ...
      const int64_t  savings           = get_balance(N(bcio.saving)).get_amount();
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_pay, bcio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...

   {
      ...
      const uint32_t initial_tot_unpaid_blocks = initial_global_state["total_unpaid_blocks"].as<uint32_t>();
      const asset    initial_supply            = get_token_supply();
      const asset    initial_bpay_balance      = get_balance(N(bcio.bpay));
      const asset    initial_vpay_balance      = get_balance(N(bcio.vpay));
      ...
      const uint32_t tot_unpaid_blocks = global_state["total_unpaid_blocks"].as<uint32_t>();
      const asset    supply            = get_token_supply();
      const asset    bpay_balance      = get_balance(N(bcio.bpay));
      const asset    vpay_balance      = get_balance(N(bcio.vpay));
...

   {
      ...
      BOOST_REQUIRE_EQUAL( error("missing authority of bcio"),
...
BOOST_FIXTURE_TEST_CASE(producers_upgrade_system_contract, bcio_system_tester) try {
   ...
   string bcio_system_wast2 = bcio_system_wast;
   string msg = "producer votes must be unique and sorted";
   auto pos = bcio_system_wast2.find(msg);
   BOOST_REQUIRE( pos != std::string::npos );
   msg[0] = 'P';
   bcio_system_wast2.replace( pos, msg.size(), msg );

   transaction trx;
   {
      auto code = wast_to_wasm( bcio_system_wast2 );
...
BOOST_FIXTURE_TEST_CASE(producer_onblock_check, bcio_system_tester) try {
...
BOOST_FIXTURE_TEST_CASE( voters_actions_affect_proxy_and_producers, bcio_system_tester, * boost::unit_test::tolerance(1e+6) ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( vote_both_proxy_and_producers, bcio_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( select_invalid_proxy, bcio_system_tester ) try {
   //accumulate proxied votes
   issue( "bob111111111", core_from_string("1000.0000"),  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "bob111111111", core_from_string("100.0002"), core_from_string("50.0001") ) );
...
BOOST_FIXTURE_TEST_CASE( double_register_unregister_proxy_keeps_votes, bcio_system_tester ) try {
   //alice1111111 becomes a proxy
   BOOST_REQUIRE_EQUAL( success(), push_action( N(alice1111111), N(regproxy), mvo()
                                                ("proxy",  "alice1111111")
                                                ("isproxy",  1)
...
BOOST_FIXTURE_TEST_CASE( proxy_cannot_use_another_proxy, bcio_system_tester ) try {
   //alice1111111 becomes a proxy
   BOOST_REQUIRE_EQUAL( success(), push_action( N(alice1111111), N(regproxy), mvo()
                                                ("proxy",  "alice1111111")
                                                ("isproxy",  1)
...
fc::mutable_variant_object config_to_variant( const bcio::chain::chain_config& config ) {
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, bcio_system_tester ) try {
   create_accounts_with_resources( {  N(defproducer1), N(defproducer2), N(defproducer3) } );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "defproducer1", 1) );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "defproducer2", 2) );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "defproducer3", 3) );

   //stake more than 15% of total BC supply to activate chain
   transfer( "bcio", "alice1111111", core_from_string("600000000.0000"), "bcio" );
...
BOOST_FIXTURE_TEST_CASE( buyname, bcio_system_tester ) try {
   ...
   BOOST_REQUIRE_EXCEPTION( create_accounts_with_resources( { N(fail) }, N(dan) ), // dan shouldn't be able to create fail
                            bcio_assert_message_exception, bcio_assert_message_is( "no active bid for name" ) );
   ...
   BOOST_REQUIRE_EXCEPTION( create_accounts_with_resources( { N(nofail) }, N(dan) ), // dan shoudn't be able to do this, sam won
                            bcio_assert_message_exception, bcio_assert_message_is( "only hibcst bidder can claim" ) );
   ...
   transfer( "bcio", "nofail", core_from_string( "1000.0000" ) );
   ...
   BOOST_REQUIRE_EXCEPTION( create_accounts_with_resources( { N(test.fail) }, N(dan) ), // dan shouldn't be able to do this
                            bcio_assert_message_exception, bcio_assert_message_is( "only suffix may create this account" ) );

   create_accounts_with_resources( { N(goodgoodgood) }, N(dan) ); /// 12 char names should succeed
} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( bid_invalid_names, bcio_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( multiple_namebids, bcio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( error("assertion failure with message: account is already hibcst bidder"),
   ...
   {
      const asset initial_names_balance = get_balance(N(bcio.names));
      BOOST_REQUIRE_EQUAL( success(),
      ...
      BOOST_REQUIRE_EQUAL( initial_names_balance + core_from_string("0.1001"), get_balance(N(bcio.names)) );
   }
   ...
   // hibcst bid is from david for prefd but no bids can be closed yet
   ...
   // changing hibcst bid pushes auction closing time by 24 hours
   BOOST_REQUIRE_EQUAL( success(),
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(prefb), N(eve) ),
                            fc::exception, fc_assert_exception_message_is( not_closed_message ) );
   // but changing a bid that is not the hibcst does not push closing time
   BOOST_REQUIRE_EQUAL( success(),
                        bidname( "carl", "prefe", core_from_string("2.0980") ) );
   produce_block( fc::hours(2) );
   produce_blocks(2);
   // bid for prefb has closed, only hibcst bidder can claim
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(prefb), N(alice) ),
                            bcio_assert_message_exception, bcio_assert_message_is( "only hibcst bidder can claim" ) );
...
BOOST_FIXTURE_TEST_CASE( vote_producers_in_and_out, bcio_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( setparams, bcio_system_tester ) try {
   //install multisig contract
   abi_serializer msig_abi_ser = initialize_multisig();
   auto producer_names = active_and_vote_producers();
   ...
   bcio::chain::chain_config params;
...
BOOST_FIXTURE_TEST_CASE( setram_effect, bcio_system_tester ) try {
