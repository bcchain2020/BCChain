...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(bcio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(bcio.token), bcio_token_wast );
   c.set_abi( N(bcio.token), bcio_token_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(bcio.token), N(create), N(bcio.token), mutable_variant_object()
              ("issuer",       "bcio" )
   ...
   cr = c.push_action( N(bcio.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "dan" )
 ...
 *  the hibcst last irreversible block over one that is longer.
 */
...
BOOST_AUTO_TEST_CASE(confirmation) try {
   ...
   BOOST_REQUIRE_EQUAL(55, blk61->bft_irreversible_blocknum); // bft irreversible number will propagate to hibcr block
