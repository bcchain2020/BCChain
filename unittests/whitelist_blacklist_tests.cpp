...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(bcio.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(bcio.token), bcio_token_wast);
         chain->set_abi(N(bcio.token), bcio_token_abi);
         chain->push_action( N(bcio.token), N(create), N(bcio.token), mvo()
              ( "issuer", "bcio.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(bcio.token), N(issue), N(bcio.token), mvo()
              ( "to", "bcio.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(bcio.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(bcio.token), N(alice)};
   test.init();

   test.transfer( N(bcio.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(bcio.token), N(bob)};
   test.init();

   test.transfer( N(bcio.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(bcio.token) );
   ...
   test.chain->set_code(N(bob), bcio_token_wast);
   test.chain->set_abi(N(bob), bcio_token_abi);

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), bcio_token_wast);
   test.chain->set_abi(N(charlie), bcio_token_abi);
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(bcio.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_bcio ) { try {
   whitelist_blacklist_tester<tester> tester1;
   tester1.init();
   tester1.chain->produce_blocks();
   tester1.chain->set_code(config::system_account_name, bcio_token_wast);
   tester1.chain->produce_blocks();
   tester1.shutdown();
...
BOOST_AUTO_TEST_CASE( blacklist_onerror ) { try {
   ...
   BOOST_CHECK_EXCEPTION( tester1.chain->produce_blocks(), fc::exception,
                          fc_exception_message_is("action 'bcio::onerror' is on the action blacklist")
                        );
