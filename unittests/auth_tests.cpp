...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for bcio reqauth action with alice's spending key
   chain.link_authority("alice", "bcio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "bcio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with bcio reqauth
   chain.unlink_authority("alice", "bcio", "reqauth");
   ...
   // Link authority for any bcio action with alice's scud key
   chain.link_authority("alice", "bcio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "bcio", "first",  "reqauth");
...
try {
   ...
   // Creating account with bcio. prefix with privileged account
   chain.create_account("bcio.test1");

   // Creating account with bcio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("bcio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'bcio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "bcio", "bcio.any", "reqauth" );
   chain.link_authority( "bob", "bcio", "bcio.any", "reqauth" );

   /// this should succeed because bcio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = bcio::chain::resource_limits::resource_usage_object;
   using by_owner = bcio::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link bcio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };
