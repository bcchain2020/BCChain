...
#include <bcio/testing/tester_network.hpp>
...
asset get_currency_balance(const TESTER& chain, account_name account) {
   return chain.get_currency_balance(N(bcio.token), symbol(SY(4,CUR)), account);
}

const std::string bcio_token = name(N(bcio.token)).to_string();
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.create_account(N(bcio.token));
   chain.produce_blocks(10);

   chain.set_code(N(bcio.token), bcio_token_wast);
   chain.set_abi(N(bcio.token), bcio_token_abi);
   ...
   chain.push_action(config::system_account_name, linkauth::get_name(), tester_account, fc::mutable_variant_object()
           ...
           ("requirement", "first"));
   chain.produce_blocks();
   chain.push_action(N(bcio.token), N(create), N(bcio.token), mutable_variant_object()
           ("issuer", bcio_token)
           ("maximum_supply", "9000000.0000 CUR")
   );


   chain.push_action(N(bcio.token), name("issue"), N(bcio.token), fc::mutable_variant_object()
           ("to",       bcio_token)
           ("quantity", "1000000.0000 CUR")
           ("memo", "for stuff")
   );

   auto trace = chain.push_action(N(bcio.token), name("transfer"), N(bcio.token), fc::mutable_variant_object()
       ("from", bcio_token)
       ("to", "tester")
   ...
   auto liquid_balance = get_currency_balance(chain, N(bcio.token));
   ...
   trace = chain.push_action(N(bcio.token), name("transfer"), N(tester), fc::mutable_variant_object()
       ("from", "tester")
       ("to", "tester2")
       ("quantity", "1.0000 CUR")
       ("memo", "hi" )
   ...
   liquid_balance = get_currency_balance(chain, N(bcio.token));
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, N(tester));
...
BOOST_AUTO_TEST_CASE(delete_auth_test) { try {
   ...
   // transfer from bcio.token to tester
   trace = chain.push_action(N(bcio.token), name("transfer"), N(bcio.token), fc::mutable_variant_object()
       ("from", "bcio.token")
       ("to", "tester")
       ("quantity", "100.0000 CUR")
       ("memo", "hi" )
...
BOOST_AUTO_TEST_CASE( mindelay_test ) { try {
   ...
   const auto& acnt = chain.control->db().get<account_object,by_name>(N(bcio.token));
   const auto abi = acnt.get_abi();
   chain::abi_serializer abis(abi, chain.abi_serializer_max_time);
   const auto a = chain.control->db().get<account_object,by_name>(N(bcio.token)).get_abi();

   string action_type_name = abis.get_action_type(name("transfer"));

   action act;
   act.account = N(bcio.token);
...
BOOST_AUTO_TEST_CASE( canceldelay_test2 ) { try {
...

   {
      ...
      trace = chain.push_action(N(bcio.token), name("transfer"), vector<permission_level>{{N(tester), N(first)}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "1.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(N(bcio.token), name("transfer"), vector<permission_level>{{N(tester), N(second)}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "5.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(N(bcio.token), name("transfer"), vector<permission_level>{{N(tester), config::owner_name}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "10.0000 CUR")
          ("memo", "hi" ),
