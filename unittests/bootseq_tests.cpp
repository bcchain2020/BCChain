...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(bcio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create bcio.msig and bcio.token
        create_accounts({N(bcio.msig), N(bcio.token), N(bcio.ram), N(bcio.ramfee), N(bcio.stake), N(bcio.vpay), N(bcio.bpay), N(bcio.saving) });

        // Set code for the following accounts:
        //  - bcio (code: bcio.bios) (already set by tester constructor)
        //  - bcio.msig (code: bcio.msig)
        //  - bcio.token (code: bcio.token)
        set_code_abi(N(bcio.msig), bcio_msig_wast, bcio_msig_abi);//, &bcio_active_pk);
        set_code_abi(N(bcio.token), bcio_token_wast, bcio_token_abi); //, &bcio_active_pk);

        // Set privileged for bcio.msig and bcio.token
        set_privileged(N(bcio.msig));
        set_privileged(N(bcio.token));

        // Verify bcio.msig and bcio.token is privileged
        const auto& bcio_msig_acc = get<account_object, by_name>(N(bcio.msig));
        BOOST_TEST(bcio_msig_acc.privileged == true);
        const auto& bcio_token_acc = get<account_object, by_name>(N(bcio.token));
        BOOST_TEST(bcio_token_acc.privileged == true);


        // Create SYS tokens in bcio.token, set its manager as bcio
        ...
        create_currency(N(bcio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to bcio.system
        issue(N(bcio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        // Set bcio.system to bcio
        set_code_abi(config::system_account_name, bcio_system_wast, bcio_system_abi);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(bcio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "bcio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), bcio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), bcio_assert_message_exception);
