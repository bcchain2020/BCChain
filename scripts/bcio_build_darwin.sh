	...
	if [ "${OS_MIN}" -lt 12 ]; then
		echo "You must be running Mac OS 10.12.x or hibcr to install BCIO."
		echo "Exiting now."
		exit 1
	fi

	if [ "${DISK_AVAIL}" -lt "$DISK_MIN" ]; then
		echo "You must have at least ${DISK_MIN}GB of available storage to install BCIO."
		echo "Exiting now."
		exit 1
	fi
	...
	then
		printf "\\tHomebrew must be installed to compile BC.IO\\n\\n"
	...
	do
		...
		(( COUNT++ ))
	done < "${SOURCE_DIR}/scripts/bcio_build_dep"
	IFS="${var_ifs}"
	...
	if [ "${BVERSION}" != "106700" ]; then
		if [ ! -z "${BVERSION}" ]; then
			...
			printf "\\BC.IO requires Boost version 1.67.\\n"
