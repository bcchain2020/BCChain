	...
	case "${OS_NAME}" in
		"Linux Mint")
		   if [ "${OS_MAJ}" -lt 18 ]; then
			   printf "\\tYou must be running Linux Mint 18.x or hibcr to install BCIO.\\n"
			   printf "\\tExiting now.\\n"
			   exit 1
		   fi
		;;
		"Ubuntu")
			if [ "${OS_MAJ}" -lt 16 ]; then
				printf "\\tYou must be running Ubuntu 16.04.x or hibcr to install BCIO.\\n"
				printf "\\tExiting now.\\n"
				exit 1
			fi
		;;
		"Debian")
			if [ $OS_MAJ -lt 10 ]; then
				printf "\tYou must be running Debian 10 to install BCIO, and resolve missing dependencies from unstable (sid).\n"
				printf "\tExiting now.\n"
				exit 1
		fi
		;;
	...
	if [ "${DISK_AVAIL%.*}" -lt "${DISK_MIN}" ]; then
		printf "\\tYou must have at least %sGB of available storage to install BCIO.\\n" "${DISK_MIN}"
		printf "\\tExiting now.\\n"
		exit 1
	fi
	...
	if [ "${COUNT}" -gt 1 ]; then
		printf "\\n\\tThe following dependencies are required to install BCIO.\\n"
		printf "\\n\\t${DISPLAY}\\n\\n" 
