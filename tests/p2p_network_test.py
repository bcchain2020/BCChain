...
# bcio should have the same key as defproducera
bcio = copy.copy(defproduceraAccount)
bcio.name = "bcio"

Print("Info of each node:")
for i in range(len(hosts)):
    node = node0
    cmd="%s %s get info" % (testUtils.Utils.GstClientPath, node.endpointArgs)
    trans = node.runCmdReturnJson(cmd)
    Print("host %s: %s" % (hosts[i], trans))


wasmFile="bcio.system.wasm"
abiFile="bcio.system.abi"
Print("\nPush system contract %s %s" % (wasmFile, abiFile))
trans=node0.publishContract(bcio.name, wasmFile, abiFile, waitForTransBlock=True)
if trans is None:
    Utils.errorExit("Failed to publish bcio.system.")
else:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, bcio)
