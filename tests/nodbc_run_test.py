...
try:
    ...
    if not dontLaunch:
        walletAccounts.append(cluster.bcioAccount)
    ...
    contractDir="contracts/bcio.token"
    wasmFile="bcio.token.wasm"
    abiFile="bcio.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getGstAccountFromDb(currencyAccount.name)
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get BC account failed", raw=True)
    ...
    account=node.getGstAccount(defproduceraAccount.name, exitOnError=True)
