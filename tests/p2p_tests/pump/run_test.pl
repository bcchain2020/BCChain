...
my $bc_home = defined $ENV{BC_HOME} ? $ENV{BC_HOME} : getcwd;
my $bcd = $bc_home . "/programs/bcd/bcd";
my $bcc = $bc_home . "/programs/bcc/bcc";

my $nodes = defined $ENV{BC_TEST_RING} ? $ENV{BC_TEST_RING} : "1";
my $pnodes = defined $ENV{BC_TEST_PRODUCERS} ? $ENV{BC_TEST_PRODUCERS} : "1";

my $prods = 21;
my $genesis = "$bc_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of bcd instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"BC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3\"]\n";

        print $cfg "plugin = bcio::producer_plugin\n";
        print $cfg "plugin = bcio::chain_api_plugin\n";
...
sub perform_work {
    ...
    if ($mode == 0) {
        ...
        my $counter = 0;
        while (time () < $stoptime) {
            `$bcc transfer bc inita 10 >> bcc.out 2>> bcc.err`;
            $counter++;
            if ($counter % 1000 == 0) {
