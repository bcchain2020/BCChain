...
killAll()
{
  programs/bcio-launcher/bcio-launcher -k 15
}

cleanup()
{
    rm -rf etc/bcio/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/clbc/clbc get info)"
  verifyErrorCode "clbc get info"
...
# stand up nodbc cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodbc \"--plugin bcio::wallet_api_plugin\" $launcherOpts
programs/bcio-launcher/bcio-launcher --nodbc "--plugin bcio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clbc/clbc --port $port2 get block 1)"
    verifyErrorCode "clbc get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clbc/clbc create key)"
verifyErrorCode "clbc create key"
...
PASSWORD_INITA="$(programs/clbc/clbc wallet create --name inita)"
verifyErrorCode "clbc wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clbc/clbc wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clbc wallet import"
programs/clbc/clbc wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clbc wallet import"
programs/clbc/clbc wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clbc wallet import"
programs/clbc/clbc wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clbc wallet import"
...
ACCOUNT_INFO="$(programs/clbc/clbc create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clbc create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clbc/clbc get account testera)"
verifyErrorCode "clbc get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clbc/clbc transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clbc transfer"
   ...
   TRANS_INFO="$(programs/clbc/clbc --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clbc get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
