...
#include <bcio/utilities/parallel_markers.hpp>
...
namespace detail {
   ...
   class authority_checker {
      ...
      public:
         ...
         {
            BC_ASSERT( static_cast<bool>(checktime), authorization_exception, "checktime cannot be empty" );
         }
      ...
      private:
         ...
         bool satisfied( const AuthorityType& authority, permission_cache_type& cached_permissions, uint16_t depth ) {
            ...
            // Check all permissions, from hibcst weight to lowest, seeing if provided authorization factors satisfies them or not
