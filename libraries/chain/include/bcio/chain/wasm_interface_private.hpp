...
#include <bcio/chain/webassembly/wavm.hpp>
#include <bcio/chain/webassembly/wabt.hpp>
...
using namespace fc;
using namespace bcio::chain::webassembly;
using namespace IR;
   ...
   struct wasm_interface_impl {
      wasm_interface_impl(wasm_interface::vm_type vm) {
         if(vm == wasm_interface::vm_type::wavm)
            ...
            BC_THROW(wasm_exception, "wasm_interface_impl fall through");
      }
      ...
      std::vector<uint8_t> parse_initial_memory(const Module& module) {
         ...
         for(const DataSegment& data_segment : module.dataSegments) {
            BC_ASSERT(data_segment.baseOffset.type == InitializerExpression::Type::i32_const, wasm_exception, "");
            BC_ASSERT(module.memories.defs.size(), wasm_exception, "");
...
#define _REGISTER_INJECTED_INTRINSIC(R, CLS, INFO)\
   BOOST_PP_CAT(BOOST_PP_OVERLOAD(_REGISTER_INTRINSIC, _UNWRAP_SEQ INFO) _EXPAND_ARGS(CLS, BCIO_INJECTED_MODULE_NAME, INFO), BOOST_PP_EMPTY())
