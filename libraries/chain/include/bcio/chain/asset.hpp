...
struct asset
{
   ...
   explicit asset(share_type a = 0, symbol id = symbol(CORE_SYMBOL)) :amount(a), sym(id) {
      BC_ASSERT( is_amount_within_range(), asset_type_exception, "magnitude of asset amount must be less than 2^62" );
      BC_ASSERT( sym.valid(), asset_type_exception, "invalid symbol" );
   }
   ...
   {
      BC_ASSERT(get_symbol() == o.get_symbol(), asset_type_exception, "addition between two different asset is not allowed");
      amount += o.amount;
      return *this;
   }
   ...
   asset& operator -= (const asset& o)
   {
      BC_ASSERT(get_symbol() == o.get_symbol(), asset_type_exception, "subtraction between two different asset is not allowed");
      amount -= o.amount;
      return *this;
   }
   ...
   friend bool operator < (const asset& a, const asset& b)
   {
      BC_ASSERT(a.get_symbol() == b.get_symbol(), asset_type_exception, "logical operation between two different asset is not allowed");
   ...
   friend asset operator - (const asset& a, const asset& b) {
      BC_ASSERT(a.get_symbol() == b.get_symbol(), asset_type_exception, "subtraction between two different asset is not allowed");
      return asset(a.amount - b.amount, a.get_symbol());
   }

   friend asset operator + (const asset& a, const asset& b) {
      BC_ASSERT(a.get_symbol() == b.get_symbol(), asset_type_exception, "addition between two different asset is not allowed");
      return asset(a.amount + b.amount, a.get_symbol());
   }
...
namespace fc {
inline void to_variant(const bcio::chain::asset& var, fc::variant& vo) { vo = var.to_string(); }
inline void from_variant(const fc::variant& var, bcio::chain::asset& vo) {
   vo = bcio::chain::asset::from_string(var.get_string());
}
}

FC_REFLECT(bcio::chain::asset, (amount)(sym))
FC_REFLECT(bcio::chain::extended_asset, (quantity)(contract) )
