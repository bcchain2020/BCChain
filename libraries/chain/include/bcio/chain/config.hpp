...
namespace bcio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(bcio);
const static uint64_t null_account_name      = N(bcio.null);
const static uint64_t producers_account_name = N(bcio.prods);
...
const static uint64_t bcio_auth_scope       = N(bcio.auth);
const static uint64_t bcio_all_scope        = N(bcio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t bcio_any_name = N(bcio.any);
const static uint64_t bcio_code_name = N(bcio.code);
...
const static bcio::chain::wasm_interface::vm_type default_wasm_runtime = bcio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace bcio::chain::config

constexpr uint64_t BC_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / bcio::chain::config::percent_100;
}

template<typename Number>
Number BC_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + bcio::chain::config::percent_100 - bcio::chain::config::percent_1)  / bcio::chain::config::percent_100;
}
