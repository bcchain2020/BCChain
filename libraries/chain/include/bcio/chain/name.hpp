...
namespace bcio { namespace chain {
   ...
   // to its 5-bit slot starting with the hibcst slot for the first char.
...
#define N(X) bcio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<bcio::chain::name> : private hash<uint64_t> {
      typedef bcio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const bcio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, bcio::chain::name& check);
} // fc


FC_REFLECT( bcio::chain::name, (value) )
