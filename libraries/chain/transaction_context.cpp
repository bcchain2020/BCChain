...
namespace bacc = boost::accumulators;
   ...
   ,pseudo_start(s)
   {
      ...
      BC_ASSERT( trx.transaction_extensions.size() == 0, unsupported_feature, "we don't support any extensions yet" );
   }

   void transaction_context::init(uint64_t initial_net_usage)
   {
      BC_ASSERT( !is_initialized, transaction_exception, "cannot initialize twice" );
      ...
      // Calculate the hibcst network usage and CPU time that all of the billed accounts can afford to be billed
   ...
   void transaction_context::exec() {
      BC_ASSERT( is_initialized, transaction_exception, "must first initialize" );
   ...
   void transaction_context::finalize() {
      ...
      // Calculate the new hibcst network usage and CPU time that all of the billed accounts can afford to be billed
      int64_t account_net_limit = 0;
      int64_t account_cpu_limit = 0;
      bool greylisted_net = false, greylisted_cpu = false;
      std::tie( account_net_limit, account_cpu_limit, greylisted_net, greylisted_cpu) = max_bandwidth_billed_accounts_can_pay();
      ...
      if (!control.skip_trx_checks()) {
         if( BOOST_UNLIKELY(net_usage > eager_net_limit) ) {
            if ( net_limit_due_to_block ) {
               BC_THROW( block_net_usage_exceeded,
               ...
               BC_THROW( greylist_net_usage_exceeded,
                          "greylisted transaction net usage is too high: ${net_usage} > ${net_limit}",
                          ("net_usage", net_usage)("net_limit", eager_net_limit) );
            } else {
               BC_THROW( tx_net_usage_exceeded,
   ...
   void transaction_context::validate_cpu_usage_to_bill( int64_t billed_us, bool check_minimum )const {
      if (!control.skip_trx_checks()) {
         if( check_minimum ) {
            const auto& cfg = control.get_global_properties().configuration;
            BC_ASSERT( billed_us >= cfg.min_transaction_cpu_usage, transaction_exception,
         ...
         if( billing_timer_exception_code == block_cpu_usage_exceeded::code_value ) {
            BC_ASSERT( billed_us <= objective_duration_limit.count(),
