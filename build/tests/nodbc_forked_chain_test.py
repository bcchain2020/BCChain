...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.bcioAccount,accounts[0],accounts[1],accounts[2],accounts[3],accounts[4]])
    ...
    for account in accounts:
        ...
        trans=node.createInitializeAccount(account, cluster.bcioAccount, stakedDeposit=0, waitForTransBlock=True, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.bcioAccount.name, account.name))
        node.transferFunds(cluster.bcioAccount, account, transferAmount, "test transfer", waitForTransBlock=True)
    ...
    # ***   Identify a hibcst block number to check while we are trying to identify where the divergence will occur   ***
    ...
    # ***   Identify the producers from the saved LIB to the current hibcst head   ***
    ...
    # ***   Analyze the producers from the saved LIB to the current hibcst head and verify they match now   ***
