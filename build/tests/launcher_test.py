...
try:
    ...
    if not dontLaunch:
        ...
        if walletMgr.launch() is False:
            ...
            errorExit("Failed to stand up bc walletd.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.bcioAccount,cluster.defproduceraAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.bcioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.bcioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.bcioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.bcioAccount, buyRAM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.bcioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.bcioAccount, buyRAM=1000000, waitForTransBlock=True, exitOnError=True)
    ...
    actualAmount=node.getAccountGstBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountGstBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
