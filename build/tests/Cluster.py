...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="bcio-ignition-wd/bootlog.txt"
    __configDir="etc/bcio/"
    __dataDir="var/lib/"
    __fileDivider="================================================================="

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kbcd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="BCtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kbcd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: bc server host
        port: bc server port
        walletHost: bc wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures bc mongo plugin
        ...
        self.bcioAccount=self.defProducerAccounts["bcio"]= Account("bcio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, extraNodgstArgs=None, useBiosBootFile=True, specificExtraNodgstArgs=None):
        """Launch cluster.
        ...
        extraNodgstArgs: string of arguments to pass through to each nodoes instance (via --nodbc flag on launcher)
        ...
        specificExtraNodgstArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodbc flags on launcher), example: { "5" : "--plugin bcio::test_control_api_plugin" }
        """
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodbc default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        nodbcArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodbcArgs += " --plugin bcio::wallet_api_plugin"
        if self.enableMongo:
            nodbcArgs += " --plugin bcio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodgstArgs is not None:
            assert(isinstance(extraNodgstArgs, str))
            nodbcArgs += extraNodgstArgs
        if Utils.Debug:
            nodbcArgs += " --contracts-console"

        if nodbcArgs:
            cmdArr.append("--nodbc")
            cmdArr.append(nodbcArgs)

        if specificExtraNodgstArgs is not None:
            assert(isinstance(specificExtraNodgstArgs, dict))
            for nodeNum,arg in specificExtraNodgstArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-nodbc")
                cmdArr.append(arg)
        ...
        self.bcioAccount=self.defProducerAccounts["bcio"]
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on bc server port %d" % (
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on bc node port: %d" % (node.port))
                return False
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.bcioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse bcio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(totalNodes, biosHost, biosPort, walletMgr, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by bcio-launcher."""
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down bc cluster.")
            return None
        ...
        bcioName="bcio"
        bcioKeys=producerKeys[bcioName]
        bcioAccount=Account(bcioName)
        bcioAccount.ownerPrivateKey=bcioKeys["private"]
        bcioAccount.ownerPublicKey=bcioKeys["public"]
        bcioAccount.activePrivateKey=bcioKeys["private"]
        bcioAccount.activePublicKey=bcioKeys["public"]
        producerKeys.pop(bcioName)

        if not walletMgr.importKey(bcioAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (bcioName))
            return None
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"bcio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init transfer")
            opts="--permission bcio@active"
            if name != "bcio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from bcio.token to %s." % (name))
                    return None
    ...
    def bootstrap(totalNodes, prodCount, totalProducers, biosHost, biosPort, walletMgr, onlyBios=False):
        ...
        trans=biosNode.publishContract(bcioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, bcioAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission bcio@active"
                    myTrans=biosNode.pushMessage("bcio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("bcio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but bcio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "bcio"
            ret=Utils.waitForBool(lam)
            if not ret:
                Utils.Print("ERROR: Block production handover failed.")
                return None

        bcioTokenAccount=copy.deepcopy(bcioAccount)
        bcioTokenAccount.name="bcio.token"
        trans=biosNode.createAccount(bcioTokenAccount, bcioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (bcioTokenAccount.name))
            return None

        bcioRamAccount=copy.deepcopy(bcioAccount)
        bcioRamAccount.name="bcio.ram"
        trans=biosNode.createAccount(bcioRamAccount, bcioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (bcioRamAccount.name))
            return None

        bcioRamfeeAccount=copy.deepcopy(bcioAccount)
        bcioRamfeeAccount.name="bcio.ramfee"
        trans=biosNode.createAccount(bcioRamfeeAccount, bcioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (bcioRamfeeAccount.name))
            return None

        bcioStakeAccount=copy.deepcopy(bcioAccount)
        bcioStakeAccount.name="bcio.stake"
        trans=biosNode.createAccount(bcioStakeAccount, bcioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (bcioStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(bcioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=bcioTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (bcioTokenAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to bcio contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (bcioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to bcio contract.")
            return None
        ...
        Utils.Print("Verify bcio issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountGstBalanceStr(bcioAccount.name)
        if expectedAmount != actualAmount:
            Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                        (expectedAmount, actualAmount))
            return None

        contract="bcio.system"
        contractDir="contracts/%s" % (contract)
        wasmFile="%s.wasm" % (contract)
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (bcioAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (bcioAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (bcioTokenAccount.name, name))
                return None
    ...
    # Kills a percentange of Gst instances starting from the tail and update bcInstanceInfos state
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodbc instances. allInstances will kill all nodbc instances running on the system."""
        ...
        if allInstances:
            # ocassionally the launcher cannot kill the bc server
            cmd="pkill -9 %s" % (Utils.GstServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down bc cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodbc instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodbc instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
