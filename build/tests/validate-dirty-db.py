...
###############################################################
# Test for validating the dirty db flag sticks repeated nodbc restart attempts
###############################################################
...
def runNodgstAndGetOutput(myTimeout=3):
    """Startup nodbc, wait for timeout (before forced shutdown) and collect output. Stdout, stderr and return code are returned in a dictionary."""
    Print("Launching nodbc process.")
    cmd="programs/nodbc/nodbc --config-dir etc/bcio/node_bios --data-dir var/lib/node_bios --verbose-http-errors --http-validate-host=false"
    Print("cmd: %s" % (cmd))
    ...
    output={}
    try:
        if debug: Print("Setting nodbc process timeout.")
        ...
        Print("ERROR: Nodgst is running beyond the defined wait time. Hard killing nodbc instance.")
...
try:
    ...
    Print("Restart nodbc repeatedly to ensure dirty database flag sticks.")
    timeout=3
