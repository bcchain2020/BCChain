...
###############################################################
# nodbc_voting_test
# --dump-error-details <Upon error print etc/bcio/node_*/config.ini and var/lib/node_*/stderr.log to stdout>
...
WalletdName=Utils.GstWalletName
ClientName="clbc"
...
try:
    ...
    maxRAMValue=1010
    extraNodgstArgs=" %s %d %s %d " % (minRAMFlag, minRAMValue, maxRAMFlag, maxRAMValue)
    if cluster.launch(onlyBios=False, pnodes=totalNodes, totalNodes=totalNodes, totalProducers=totalNodes, extraNodgstArgs=extraNodgstArgs, useBiosBootFile=False) is False:
        Utils.cmdError("launcher")
        errorExit("Failed to stand up bc cluster.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.bcioAccount])
    ...
    # create accounts via bcio as otherwise a bid is needed
    for account in accounts:
        Print("Create new account %s via %s" % (account.name, cluster.bcioAccount.name))
        trans=nodes[0].createInitializeAccount(account, cluster.bcioAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
        transferAmount="70000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.bcioAccount.name, account.name))
        nodes[0].transferFunds(cluster.bcioAccount, account, transferAmount, "test transfer")
    ...
    Print("Create new account %s via %s" % (contractAccount.name, cluster.bcioAccount.name))
    trans=nodes[0].createInitializeAccount(contractAccount, cluster.bcioAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
    transferAmount="90000000.0000 {0}".format(CORE_SYMBOL)
    Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.bcioAccount.name, contractAccount.name))
    nodes[0].transferFunds(cluster.bcioAccount, contractAccount, transferAmount, "test transfer")
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    timeOutCount+=1
                    if timeOutCount>=3:
                       Print("Failed to push create action to bcio contract for %d consecutive times, looks like nodbc already exited." % (timeOutCount))
                       keepProcessing=False
                       break
                    Print("Failed to push create action to bcio contract. sleep for 60 seconds")
                    time.sleep(60)
                else:
                    timeOutCount=0
                time.sleep(1)
