...
def stageScenario(stagedNodeInfos):
    ...
    for stagedNodeInfo in stagedNodeInfos:
        configPath=os.path.join(stagingDir, "etc/bcio/node_%02d" % (count))
...
parser.add_argument("--dump-error-details",
                    help="Upon error print etc/bcio/node_*/config.ini and var/lib/node_*/stderr.log to stdout",
                    action='store_true')
...
killGstInstances= not args.dont-kill
...
def myTest(transWillEnterBlock):
    ...
    try:
        ...
        if cluster.launch(pnodes, total_nodes, topo, delay) is False:
            error("Failed to stand up bc cluster.")
            return False
