...
###############################################################
# nodbc_run_remote_test
#  Tests remote capability of the nodbc_run_test. Test will setup cluster and pass nodes info to nodbc_run_test. E.g.
#  nodbc_run_remote_test.py -v --clean-run --dump-error-detail
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodbc_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodbc", actualTest))
    Print("cmd: %s\n" % (cmd))
