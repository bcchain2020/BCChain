/**
 *  @file
 *  @copyright defined in bcio/LICENSE.txt
 */
...
{
   try {
      app().set_version(bcio::nodbc::config::version);
      app().register_plugin<history_plugin>();

      auto root = fc::app_path();
      app().set_default_data_dir(root / "bcio/nodbc/data" );
      app().set_default_config_dir(root / "bcio/nodbc/config" );
      ...
      initialize_logging();
      ilog("nodbc version ${ver}", ("ver", app().version_string()));
      ilog("bcio root is ${root}", ("root", root.string()));
      ilog("nodbc using configuration file ${c}", ("c", app().full_config_file_path().string()));
      ilog("nodbc data directory is ${d}", ("d", app().data_dir().string()));
      app().startup();
      app().exec();
