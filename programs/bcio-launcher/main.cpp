...
#include <net/if.h>
#include <bcio/chain/genesis_state.hpp>
...
void
bcd_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string bcio_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), bcio_home(), instances(1) {}
};
...
struct launcher_def {
   ...
   string bcd_extra_args;
   std::map<uint,string> specific_nodbc_args;
   testnet_def network;
   string gelf_endpoint;
   ...
   bcio::chain::genesis_state genesis_from_file;

   void assign_name (bcd_def &node, bool is_bios);
   ...
   void prep_remote_config_dir (bcd_def &node, host_def *host);
   void launch (bcd_def &node, string &gts);
   void kill (launch_modes mode, string sig_opt);
   static string get_node_num(uint16_t node_num);
   pair<host_def, bcd_def> find_node(uint16_t node_num);
   vector<pair<host_def, bcd_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "nodbc does not require transaction signatures.")
    ("nodbc", bpo::value<string>(&bcd_extra_args), "forward nodbc command line argument(s) to each instance of nodbc, enclose arg(s) in quotes")
    ("specific-num", bpo::value<vector<uint>>()->composing(), "forward nodbc command line argument(s) (using \"--specific-nodbc\" flag) to this specific instance of nodbc. This parameter can be entered multiple times and requires a paired \"--specific-nodbc\" flag")
    ("specific-nodbc", bpo::value<vector<string>>()->composing(), "forward nodbc command line argument(s) to its paired specific instance of nodbc(using \"--specific-num\"), enclose arg(s) in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many nodbc instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if (vmap.count("specific-num")) {
    ...
    const auto specific_args = vmap["specific-nodbc"].as<vector<string>>();
    if (specific_nums.size() != specific_args.size()) {
      cerr << "ERROR: every specific-num argument must be paired with a specific-nodbc argument" << endl;
      exit (-1);
    }
    ...
    for(uint i = 0; i < specific_nums.size(); ++i)
    {
      ...
      if (num >= total_nodes) {
        cerr << "\"--specific-num\" provided value= " << num << " is hibcr than \"--nodes\" provided value=" << total_nodes << endl;
        exit (-1);
      }
      specific_nodbc_args[num] = specific_args[i];
    }
  }
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &bcd : binding.instances) {
          bcd.host = binding.host_name;
          bcd.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(bcd.p2p_port);

          aliases.push_back (bcd.name);
        }
      }
  ...
  char* erd_env_var = getenv ("BCIO_HOME");
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable BCIO_HOME is set to the build path." << endl;
    exit (-1);
  }
...
void
launcher_def::assign_name (bcd_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.bcio_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      bcd_def bcd;
      assign_name(bcd, i == 0);
      aliases.push_back(bcd.name);
      bcd.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(bcd));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->bcio_home =
          (local_id.contains (lhost->host_name) || servers.default_bcio_home.empty()) ?
          erd : servers.default_bcio_home;
        host_ndx++;
      } // ph_count == 0

      bcd_def bcd;
      assign_name(bcd, do_bios);
      bcd.has_db = false;
      ...
      aliases.push_back(bcd.name);
      bcd.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(bcd));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  bcd_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->bcio_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->bcio_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->bcio_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->bcio_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->bcio_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = bcio::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = bcio::mongo_db_plugin\n";
  }
  if ( p2p == p2p_plugin::NET ) {
    cfg << "plugin = bcio::net_plugin\n";
  } else {
    cfg << "plugin = bcio::bnet_plugin\n";
  }
  cfg << "plugin = bcio::chain_api_plugin\n"
      << "plugin = bcio::history_api_plugin\n";
  cfg.close();
}
...
launcher_def::init_genesis () {
   ...
   if (!bfs::exists(genesis_path)) {
      ...
      bcio::chain::genesis_state default_genesis;
      fc::json::save_to_file( default_genesis, genesis_path, true );
   }
   string bioskey = string(network.nodes["bios"].keys[0].get_public_key());

   fc::json::from_file(genesis_path).as<bcio::chain::genesis_state>(genesis_from_file);
...
void
launcher_def::prep_remote_config_dir (bcd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->bcio_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->bcio_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->bcio_home;

  cmd = "cd " + host->bcio_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->bcio_home
...
void
launcher_def::launch (bcd_def &instance, string &gts) {
  ...
  string bcdcmd = "programs/nodbc/nodbc ";
  if (skip_transaction_signatures) {
    bcdcmd += "--skip-transaction-signatures ";
  }
  if (!bcd_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +bcio::mongo_db_plugin");
       string args = std::regex_replace (bcd_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       bcdcmd += args + " ";
    }
    else {
       bcdcmd += bcd_extra_args + " ";
    }
  }
  if (instance.name != "bios" && !specific_nodbc_args.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodbc_args.count(node_num)) {
        bcdcmd += specific_nodbc_args[node_num] + " ";
     }
  }

  if( add_enable_stale_production ) {
    bcdcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  bcdcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  bcdcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    bcdcmd += " --genesis-timestamp " + gts;
  }

  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->bcio_home + "; nohup " + bcdcmd + " > "
    ...
    string cmd = "cd " + host->bcio_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << bcdcmd << endl;

    bp::child c(bcdcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << bcdcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(bcd_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, bcd_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, bcd_def>> node_list;
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const bcd_def& node = node_pair.second;
      const string node_num = node.get_node_num();
      cout << "Bouncing " << node.name << endl;
      string cmd = "./scripts/bcio-tn_bounce.sh " + bcd_extra_args;
      if (node_num != "bios" && !specific_nodbc_args.empty()) {
         const auto node_num_i = boost::lexical_cast<uint16_t,string>(node_num);
         if (specific_nodbc_args.count(node_num_i)) {
            cmd += " " + specific_nodbc_args[node_num_i];
         }
      }

      do_command(host, node.name, { { "BCIO_HOME", host.bcio_home }, { "BCIO_NODE", node_num } }, cmd);
   }
}
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/bcio-tn_down.sh ";
      do_command(host, node.name,
                 { { "BCIO_HOME", host.bcio_home }, { "BCIO_NODE", node_num }, { "BCIO_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
...
launcher_def::roll (const string& host_names) {
   ...
   for (string host_name: hosts) {
      ...
      string cmd = "./scripts/bcio-tn_roll.sh ";
      do_command(*host, host_name, { { "BCIO_HOME", host->bcio_home } }, cmd);
   }
}
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the bcio-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the bcio-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the bcio-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << bcio::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(bcio_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (bcio_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_bcio_home) (ssh) )
